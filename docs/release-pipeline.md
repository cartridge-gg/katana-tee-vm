...
`Initrd Test` workflow) exercises most pins on every PR; OVMF pin bumps are
only exercised by the release workflow itself, so prefer a `workflow_dispatch`
dry run before tagging.

**Dry-run the pipeline** without publishing: `gh workflow run release.yml -f
...
- `Initrd Test` (every PR / main push) builds the sealed initrd with the same
...
- `Lint` shellchecks all build scripts plus the guest init (as POSIX sh);
  `snp-tools` runs the crate's tests, which pin the key-derivation contract
